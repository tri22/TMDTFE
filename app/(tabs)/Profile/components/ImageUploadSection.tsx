...
export default function ImageUploadSection({ onchange }) {
    ...
    //     const fileName = uri.split('/').pop(); // Upload ảnh lên server 
    //     const formData = new FormData(); formData.append('file', {
    //         uri, name: fileName, type: 'image/jpeg', // Hoặc loại phù hợp 
    //     }); try {
    ...
    //         updatedImages[index] = uri; // Hiển thị trên giao diện 
    //         updatedUrls[index] = data.url; // Lưu URL từ server 
    //         setImages(updatedImages); setImageUrls(updatedUrls);
    //         onchange?.(updatedUrls.filter((url) => url)); //Gửi danh sách URL hợp lệ 
    //     } catch (error) {
    ...
    const handleItemPress = async (index) => {
        ...
        updatedUrls[index] = uri; // Không upload, chỉ lưu uri cục bộ

        setImages(updatedImages);
        setImageUrls(updatedUrls);

        onchange?.(updatedUrls.filter((url) => url));
    };
    return (
        <View style={styles.imageContainer}>
            ...
            {imageUrls.map((url, index) => (url && (
                <Text key={index}
                    style={styles.fileInfo}> Ảnh {index + 1}:
                </Text>
            )))}
